$(document).ready(function(){
	...
	var printExamples = function() {
		...
		for (j = 0; j < 4; iterator++, j++) {
			if (iterator === foods.length) {			// if iterator reach end of food list, stop the function
				alert('no more foods');
				break;
	...
	// function to care about common foods
	var findCommonFood = function(common_food) {
		iterator = 0;
		foods = [];
		nowShowing = [];
		for (i in allFoods) {
			food = allFoods[i];
			// if there is a food has name that contains common_food (ex. pizza) push to list
			if (food.name.toLowerCase().search(common_food) > -1) 
				foods.push(food);
			else {
				continue;
			}
